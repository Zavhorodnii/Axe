...
class WC_Yandex_Money_Gateway_card extends WC_Payment_Gateway{
	function __construct() {
		$this->id = 'wc_yandex_money_gateway_card';
		$this->method_title = __( 'WC Yandex.Money (card)');
		$this->method_description = __( 'Оплата с помощью банковских карт (без привязки кошельку)');
		$this->title = __( 'WC Yandex.Money Gateway card');
	...
	public function init_form_fields() {
		$this->form_fields = array(
			...
			'title' => array(
				...
				'default'	=> __( 'Банковской картой'),
			),
			'description' => array(
				'title'		=> __( 'Description'),
				'type'		=> 'textarea',
				'desc_tip'	=> __( 'Описание оплаты клиент будет видеть в процессе оформления заказа.'),
				'default'	=> __( 'Оплата банковской картой через Яндекс.Деньги.')
			),
			'wallet_number' => array(
